...
PouchDB also offers separate browser plugins that use backends other than IndexedDB and WebSQL. These plugins are all tested and pass our unit test suite at 100%. They do add a hefty footprint due to Browserify dependencies, though, so take them with a grain of salt.

{% include alert_start.html variant="warning"%}
Currently the plugins do not work with Browserify itself; you have to include them as separate scripts in your HTML page.
{% include alert_end.html%}
